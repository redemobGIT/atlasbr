...
def fetch_tracts_raw(munis: Iterable[int], year: int) -> gpd.GeoDataFrame:
    ...
    logger.info(f"Fetching Census Tracts for {len(muni_list)} municipalities (Year {year})...")
    ...
    if not dfs:
        raise RuntimeError("No census tracts found for the requested municipalities.")
