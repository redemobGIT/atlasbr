...
from roda.defaults.ibge import BD_TABLE_SETOR_2022, INCOME_2022_URL, CENSO_RACES, CFG
...
def import_census_data(
    ...
    *,
    ...
    frames = [
        _census_year(y, muni_codes, gcp_billing, themes, clip_urban)
        for y in (2010, 2022)
    ]
    return pd.concat(frames).set_index("year", append=True)  # MultiIndex: (tract, year)
...
def _query_theme(
    theme: str, year: int, munis: tuple[int], billing: str
) -> pd.DataFrame:
...
def _pull_bq(
    table: str, cols: dict[str, str], munis: Iterable[int], billing: str
) -> pd.DataFrame:
    ...
    """
    return bd.read_sql(sql, billing_project_id=billing).set_index("id_setor_censitario")
...
def _tracts(
    ...
    *,
    ...
    gdf = (
        gdf.assign(
            id_setor_censitario=lambda d: d.code_tract.astype("int64")
            .astype(str)
            .str.zfill(15)
    ...
    if clip_to_urban:
        ...
        gdf = gpd.overlay(gdf.reset_index(), mask, how="intersection").set_index(
            "id_setor_censitario"
        )
...
def _urban_mask(
    year: int, crs: str, bbox: tuple[float, float, float, float]
) -> gpd.GeoDataFrame:
    """Cria uma máscara de área urbana dissolvida e com buffer."""
    minx, miny, maxx, maxy = bbox

    urban = (
        get_urban_area_gdf(year).to_crs(crs).cx[minx:maxx, miny:maxy].geometry.buffer(0)
    )
...
def _income_2022(*_) -> pd.DataFrame:
    ...
    with zipfile.ZipFile(
        BytesIO(requests.get(INCOME_2022_URL, timeout=60).content)
    ) as zf:
    ...
    return (
        ...
        .assign(
            id_setor_censitario=lambda d: d.id_setor_censitario.str.zfill(15),
            rendimento_medio=lambda d: pd.to_numeric(
                d.rendimento_medio.str.replace(",", ".").replace(
                    {"X": np.nan, ".": np.nan}
                ),
                errors="coerce",
            ),
...
def _race_2022(muni_ids: tuple[int], billing: str) -> pd.DataFrame:
    ...
    race_block = lambda off: _sql_sum(
        (b + off for b in range(657, 717, 5)), prefix="IFNULL(V", suffix=",0)"
    )
    races_15p_sql = ",\n".join(
        f"({race_block(i)}) AS race_{r}_15p" for i, r in enumerate(CENSO_RACES)
    )
    sql = f"""
    ...
    race_cols = [f"race_{r}_15p" for r in CENSO_RACES]
    grouped = df.groupby("id_mun")[race_cols + ["pop_15p"]].sum()
    muni_share = grouped[race_cols].div(grouped["pop_15p"], axis=0)
    for r in CENSO_RACES:
        df[f"race_{r}"] = df[f"race_{r}_15p"] + df["age_0_14"] * df["id_mun"].map(
            muni_share[f"race_{r}_15p"]
        )
    return df.set_index(df.id_setor_censitario.str.zfill(15))[
        [f"race_{r}" for r in CENSO_RACES]
    ]
