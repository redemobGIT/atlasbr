...
CENSUS_CATALOG: List[CensusThemeSpec] = [
    ...
    CensusThemeSpec(
        ...
        required_columns=["v022"] + 
                         _gen_cols("v", 35, 135, width=3)
    ),
    CensusThemeSpec(
        theme="race",
        year=2010,
    ...
    CensusThemeSpec(
        ...
        required_columns=["pessoas", "V00644"] + 
                         _gen_cols("V", 645, 657, width=5)
    ),
    CensusThemeSpec(
        theme="race",
        year=2022,
...
def get_theme_spec(theme: str, year: int) -> CensusThemeSpec:
    """Retrieves the configuration for a given theme and year."""
    for spec in CENSUS_CATALOG:
        if spec.theme == theme and spec.year == year:
            return spec
    raise ValueError(f"No configuration found in CENSUS_CATALOG for theme='{theme}', year={year}")